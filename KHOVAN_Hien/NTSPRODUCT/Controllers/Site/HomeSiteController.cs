...
namespace NTSPRODUCT.Controllers.Site
{
    public class HomeSiteController : Controller
    {
        ...
        public ActionResult SiteMap()
        {
            ...
            //danh muc moduler
            var cateP = db.Categorys.Where(s => s.cateLang.Equals(lang) && s.cateActive == true && s.cateType == ClassExten.typePro).OrderByDescending(s => s.cateOrder).ToList();
            ...
            //danh muc thiet bi
            var cateP2 = db.Categorys.Where(s => s.cateLang.Equals(lang) && s.cateActive == true && s.cateType == ClassExten.typeDevice).OrderByDescending(s => s.cateOrder).ToList();
            foreach (var item in cateP2)
            {
            ...
            {
                text += "<url>" + Environment.NewLine + "\t<loc>http://" + urlHost +(item.pType.Equals(Constants.Product)? "/san-pham/" : "/thiet-bi/")  + item.pro_key + "</loc>" + Environment.NewLine + "\t<changefreq>" + changeFreq + "</changefreq>" + Environment.NewLine + "\t<priority>" + productPriority + "</priority>" + Environment.NewLine + "</url>" + Environment.NewLine;
            }
            //nhom tin
        ...
        public ActionResult App()
        {
            ...
            ViewBag.img = ClassExten.GetUrlHost() + conf.logoTop;
            #endregion
            var app = db.FieldActivities.Where(u => u.slang.Equals(lang) && u.active == true).OrderBy(u => u.numberOder).ToList();
            return View(app);
        }
        public ActionResult Solution(string id)
        {
            ...
            if (string.IsNullOrEmpty(id))
            {
                ...
                ViewBag.img = ClassExten.GetUrlHost() + conf.logoTop;
                #endregion
                var sl = db.Solutions.Where(u => u.lang.Equals(lang) && u.solutionActive == true).OrderBy(u => u.solutionOrder).ToList();
                ViewBag.sl = sl;
                return View();
            ...
            {
                var data = db.Solutions.FirstOrDefault(u => u.lang.Equals(lang) && u.solutionActive == true && u.name_url.Equals(id));
                #region[load seo]  
                ViewBag.title = data.titleSeo;
                ViewBag.description = data.desSeo;
                ViewBag.keywords = data.keySeo;
                ViewBag.url = HttpContext.Request.Url.AbsoluteUri;
                ViewBag.img = ClassExten.GetUrlHost() + data.solutionImage;
                #endregion
                return View(data);
